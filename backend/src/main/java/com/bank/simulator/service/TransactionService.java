...
public class TransactionService {
    ...
    /**
     * Core money transfer method. Preserves all original business rules:
     * - Self-transfer prevention
     * - 6-digit PIN validation & match
     * - Active status checks (both accounts)
     * - Sufficient balance check
     * - Atomic @Transactional (replaces manual conn.commit/rollback)
     * - TXN_YYYYMMDDNNN ID format
     * - Email notifications sent AFTER DB commit via @Async
     */
    @Transactional
    ...
    /**
     * Generates TXN_YYYYMMDDNNN format. Thread-safe via synchronized block.
     * Example: TXN_20260405001, TXN_20260405002, ...
     */
